						...
						if (children && children.length > 1) {
							...
							if (pb && pb.length > 0) {
								...
								window.parent.postMessage(
									JSON.stringify({ type: "selection", action: action, id: obj.props.value, element: type, x: rect.x, y: rect.y, w: rect.width, h: rect.height }),
									"*"
								);
							}
						} else {
				...
				for (const obj of pom.layout) {
					...
					if (obj.layout && obj.layout.length > 0) {
						if (obj.name === "Artboard") {
							...
							for (const item of obj.layout[0].rowWidgets) {
								if (item.props.kind === "TextAreaInput") {
									let text = item.props.value;
									let data = JSON.stringify({ type: "textValue", text: text });
									window.parent.postMessage(data, "*");
								}
							}
						}
